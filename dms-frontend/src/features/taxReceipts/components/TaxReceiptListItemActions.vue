<template>
  ...
  <QMenu v-model="showAction" style="min-width: 125px; border-radius: 6px">
    <QList dense class="q-pa-xs">
      ...
      <QItem
        v-if="hasFullVisualAccess && isCanceled"
        clickable
        v-close-popup
        class="text-black"
        style="padding: 8px 12px; border-radius: 6px"
      ...
      <QItem
        v-else-if="hasFullVisualAccess && canCancel"
        clickable
        v-close-popup
        class="text-red-8"
        style="padding: 8px 12px; border-radius: 6px"
...
const showActions = computed(() => {
  return (
    ...
    (props.hasFullVisualAccess && (props.taxReceipt.status === 'COMPLETED' || isCanceled.value))
  )
})
</script>
